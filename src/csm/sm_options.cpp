...
void sm_options(struct sm_params*p, struct option*ops) {
    ...
    options_double(ops, "max_reading", &(p->max_reading), 10.0, "Don't use readings longer than max_reading (m)");
    ...
    options_double(ops, "kf_dist_linear", &(p->kf_dist_linear), 0.1, "delta distance to add a new keyframe, default: 0.1 meter");
    options_double(ops, "kf_dist_angular", &(p->kf_dist_angular), 0.174532922, "delta angle to add a new keyframe, default: 10 degress");
