...
/** 误差函数: e = ∑[(R(theta)*p_i + x - p_j1)*n_i]^2,
 *  推导公式见图片 icp_covariance.jpg
 */
		...
		val *cov0_x, val *dx_dy1, val *dx_dy2)
{
    ...
    // 对变换x分成平移t和旋转theta两部分来求2阶导数
    val d2J_dt2       = zeros(2,2); // t = [delta_x, delta_y]^T
    val d2J_dt_dtheta = zeros(2,1); // x = [t, theta]^T
    ...
    for(i=0; i<laser_sens->nrays; i++) {
        ...
        // v5 = R"(theta)*p_i = dv1_dtheta
        val v5 = m(rot(theta+M_PI), p_i);
        val I = ones(1, 1);
        val d2J_dtheta2_k = sc(2.0, sum(I, sum(m3(tr(v2),C_k, v5), m3(tr(v1),C_k,v1))));
//		val d2J_dtheta2_k = sc(2.0, sum( m3(tr(v2),C_k, v_new), m3(tr(v1),C_k,v1)));
        ...
        val d2Jk_dt_drho_i = sc(2.0, m(C_k,v3));
        val d2Jk_dtheta_drho_i = sc(2.0, sum( m3(tr(v2),C_k,v4), m3(tr(v3),C_k,v1)));
        add_to_col(d2J_dxdy2, (size_t)i, comp_col(d2Jk_dt_drho_i, d2Jk_dtheta_drho_i));
    ...
    val edx_dy1 = sc(-1.0, m(inv(d2J_dx2), d2J_dxdy1)); // 3xN
    val edx_dy2 = sc(-1.0, m(inv(d2J_dx2), d2J_dxdy2)); // 3xN
...
//    *cov0_x = egsl_promote(inv(ecov0_x));
//    *cov0_x = egsl_promote(inv(d2J_dx2));
//    *cov0_x = egsl_promote(d2J_dx2);
